...
<div class="m-portlet m-portlet--mobile">
    @if (Auth::user()->child()->first() instanceof \App\Admin)
        <form id="m_admin_edit_staff" action="{{ route('staff.update', [$store_id, $staff->id]) }}" method="post" class="m-form m-form--label-align-right">
    @elseif (Auth::user()->child()->first() instanceof \App\Owner)
        <form id="m_owner_edit_staff" action="{{ route('staff.update', [$store_id, $staff->id]) }}" method="post" class="m-form m-form--label-align-right">
    @endif
        <div class="m-portlet__body">
            <div class="form-group m-form__group row {{ $errors->first('username') ? 'has-danger' : '' }}">
                ...
                <div class="col-lg-6">
                    <input type="text" name="username" class="form-control m-input" placeholder="Enter username" value="{{ old('username') ? old('username') : explode('_', $staff->accounts()->first()->username)[1] }}" aria-describedby="username-error">
            ...
            <div class="form-group m-form__group row {{ $errors->first('email') ? 'has-danger' : '' }}">
                ...
                <div class="col-lg-6">
                    <input type="email" name="email" class="form-control m-input" placeholder="Enter email" value="{{ old('email') ? old('email') : $staff->accounts()->first()->email }}" aria-describedby="email-error">
            ...
            <div class="form-group m-form__group row {{ $errors->first('name') ? 'has-danger' : '' }}">
                ...
                <div class="col-lg-6">
                    <input type="text" name="name" class="form-control m-input" placeholder="Enter name" value="{{ old('name') ? old('name') : $staff->accounts()->first()->name }}" aria-describedby="name-error">
            ...
            <div class="form-group m-form__group row {{ $errors->first('phone') ? 'has-danger' : '' }}">
                ...
                <div class="col-lg-6">
                    <input type="text" name="phone" class="form-control m-input" placeholder="Enter phone" value="{{ old('phone') ? old('phone') : $staff->accounts()->first()->phone }}" aria-describedby="phone-error">
                        ...
                        <div class="m-form__control">
                            <select class="form-control" name="status_id" aria-describedby="status_id-error">
                                @foreach ($statuses as $status)
                                    <option value="{{ $status->id }}" {{ old('status_id') ? ($status->id == old('status_id') ? 'selected' : '') : ($status->id == $staff->accounts()->first()->status()->first()->id ? $staff->accounts()->first()->status()->first()->id : '') }}>
                                        {{ $status->name }}
                                    </option>
                                @endforeach
                            </select>
