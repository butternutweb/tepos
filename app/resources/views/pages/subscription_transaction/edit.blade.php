...
@section('title', 'Edit Subscription Transaction')
...
<div class="m-portlet m-portlet--mobile">
    <form id="m_subscription_transaction" action="{{ route('subs-trans.update', $subscription_transaction->id) }}" method="post" class="m-form m-form--label-align-right">
        <div class="m-portlet__body">
            <div class="form-group m-form__group row {{ $errors->first('date') ? 'has-danger' : '' }}">
                ...
                <div class="col-lg-6">
                    <div class="input-group date" id="date">
                        <input type="text" name="date" class="form-control m-input" readonly="true" value="{{ old('date') ? old('date') : explode(' ', $subscription_transaction->date)[0] }}" placeholder="Enter date" aria-describedby="date-error">
            ...
            <div class="form-group m-form__group row {{ $errors->first('payment_method') ? 'has-danger' : '' }}">
                ...
                <div class="col-lg-6">
                    <input type="text" name="payment_method" class="form-control m-input" placeholder="Enter payment method" value="{{ old('payment_method') ? old('payment_method') : $subscription_transaction->payment_method }}" aria-describedby="payment_method-error">
                        ...
                        <div class="m-form__control">
                            <select class="form-control" name="owner_id" aria-describedby="owner_id-error">
                                @foreach ($owners as $owner)
                                    <option value="{{ $owner->id }}" {{ old('owner_id') ? ($owner->id == old('owner_id') ? 'selected' : '') : ($owner->id == $subscription_transaction->owner()->first()->id ? 'selected' : '') }}>
                    ...
                    <div class="m-form__control">
                        <select class="form-control" name="subscription_plan_id" aria-describedby="subscription_plan_id-error">
                            @foreach ($subscription_plans as $subscription_plan)
                                <option value="{{ $subscription_plan->id }}" {{ old('subscription_plan_id') ? ($subscription_plan->id == old('subscription_plan_id') ? 'selected' : '') : ($subscription_plan->id == $subscription_transaction->plan()->first()->id ? 'selected' : '') }}>
